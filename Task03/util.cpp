...
void print_test(int matrix[DEFAULT_SIZE][DEFAULT_SIZE], int n, int m, int expected, string test_name) {
	HANDLE h;
	...
	int actual = count_rows_with_more_positive_values(matrix, n, m);
	...
	if (!result) {
		cout << "Result count: expected = " << expected
